...
/**
 * Marks a person's attendance for a week in a course as false.
 */
public class UnattendCommand extends Command {

    public static final String COMMAND_WORD = "unattend";

    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Unmarks attendance for a person.\n"
            + "Parameters: INDEX (must be a positive integer) "
            + PREFIX_COURSE + "COURSE_CODE "
            + PREFIX_WEEK + "WEEK\n"
            + "Example: " + COMMAND_WORD + " 1 " + PREFIX_COURSE + "CS2103T " + PREFIX_WEEK + "1";

    public static final String MESSAGE_SUCCESS = "Unmarked attendance for student at index %1$s: %2$s did "
            + "not attend %3$s %4$s Week %5$s's tutorial";
    ...
    /**
     * Creates an UnattendCommand to unmark the person's attendance at {@code index} for {@code courseCode}
     * on {@code week}.
     */
    public UnattendCommand(Index index, String courseCode, int week) {
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        TutInfo updatedTutInfo = tutInfoToUpdate.setAttendance(week, false);
        ...
        model.setPersonToShow(editedPerson); // Always show the unattended person
